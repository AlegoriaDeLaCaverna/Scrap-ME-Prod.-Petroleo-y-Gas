...
st.markdown("Datos crudos extraídos del Ministerio de Energía. (Barriles estandarizados para Oil).")
...
# Variable booleana para identificar si estamos operando con petróleo
...
if st.button("Procesar Datos y Generar Excel", type="primary"):
    ...
    # --- LA INGENIERÍA FINANCIERA (CONVERSIÓN DE BARRLES) ---
    factor_conversion = 6.28981 / 1000

    if se_puede_desglosar:
        # Sumamos los m3 crudos
        suma_m3 = df_agrupado[cols_grafico].sum(axis=1)
        
        if es_petroleo:
            # Creamos la columna convertida
            df_agrupado['Total Consolidado (Miles de bbl)'] = suma_m3 * factor_conversion
            # Opcional: redondeamos a 2 decimales para estética
            df_agrupado['Total Consolidado (Miles de bbl)'] = df_agrupado['Total Consolidado (Miles de bbl)'].round(2)
        else:
            df_agrupado['Total Consolidado'] = suma_m3
    else:
        # Si NO se desglosa (Promedio Diario) y es petróleo, machacamos la columna única
        if es_petroleo:
            col_unica = cols_grafico[0]
            df_agrupado[col_unica] = (df_agrupado[col_unica] * factor_conversion).round(2)
            unidad_medida = "Miles de bbl/día" # Actualizamos la etiqueta del eje Y dinámicamente
    ...
    encabezados = ['Período'] + [str(col).replace('_', ' ').title() for col in cols_grafico]
    if se_puede_desglosar:
        if es_petroleo:
            encabezados.append('Total Consolidado (Miles de bbl)')
        else:
            encabezados.append('Total Consolidado')
    ...
    chart = LineChart()
    chart.title = f"{nombre_fluido} - {empresa_elegida}"
    chart.style = 13
    chart.width = 24  
    chart.height = 12 
    chart.x_axis.title = "Período"
    chart.y_axis.title = f"Volumen ({unidad_medida})"
    ...
    datos_ref = Reference(ws, min_col=2, min_row=1, max_col=1 + len(cols_grafico), max_row=ws.max_row)
