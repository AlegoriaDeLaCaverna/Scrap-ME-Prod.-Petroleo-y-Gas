...
# --- VARIABLES DE ESTADO PARA EL CONSTRUCTOR DE CONSORCIOS ---
...
with col2:
    ...
    modo_analisis = st.radio("⚔️ Estrategia Corporativa:", ["Consolidar (Sumar total)", "Comparar Grupos/Empresas"])
    
    grupos_definidos = {}
    empresas_elegidas_consolidar = []
    
    if modo_analisis == "Consolidar (Sumar total)":
        empresas_elegidas_consolidar = st.multiselect(
            "🏭Seleccione Empresas a consolidar (Deje vacío para analizar Todas):", 
        ...
        st.markdown("### Constructor de Competidores")
        st.caption("Funda personerías jurídicas bajo un mismo nombre para ponerlas a competir.")
        
        c_btn1, c_btn2 = st.columns(2)
        with c_btn1:
            if st.button("➕ Agregar Competidor"):
                st.session_state.num_grupos += 1
        with c_btn2:
            if st.button("➖ Quitar Competidor") and st.session_state.num_grupos > 1:
                st.session_state.num_grupos -= 1

        for i in range(st.session_state.num_grupos):
            st.markdown(f"**Competidor {i+1}**")
            cx1, cx2 = st.columns([1, 2])
            with cx1:
                nombre_g = st.text_input("Nombre en reporte:", key=f"nom_{i}", placeholder=f"Ej: Grupo {i+1}")
            with cx2:
                sociedades_g = st.multiselect("Sociedades que lo componen:", options=lista_empresas_reales, key=f"emps_{i}")
...
if st.session_state.procesar_clicked:
    ...
    # =========================================================================
    # RAMA 1: COMPARACIÓN COMPETITIVA DE GRUPOS
    # =========================================================================
    if modo_analisis == "Comparar Grupos/Empresas":
        ...
        # Agrupamos por mes y por el nuevo nombre del grupo
        df_agrupado = df_filtrado.groupby(['Periodo', 'Competidor'])[col_valor].sum().reset_index()
        # Pivotamos para que cada grupo sea una columna
        df_agrupado = df_agrupado.pivot(index='Periodo', columns='Competidor', values=col_valor).reset_index().fillna(0)
        df_agrupado.columns.name = None
        
        cols_grupos = [c for c in df_agrupado.columns if c != 'Periodo']
        cols_convertidas = []
        
        for grp in cols_grupos:
            col_conv = f"{grp} ({acronimo_final})"
            df_agrupado[col_conv] = (df_agrupado[grp] * factor_conversion).round(2)
            cols_convertidas.append(col_conv)
            
        columnas_finales = ['Periodo'] + cols_grupos + cols_convertidas
        ...
        if len(grupos_definidos) > 3: 
            nombre_empresa_reporte += " y otros"

    # =========================================================================
    # RAMA 2: CONSOLIDACIÓN CLÁSICA
    # =========================================================================
    else:
        if not empresas_elegidas_consolidar:
            ...
            if len(empresas_elegidas_consolidar) == 1:
                nombre_empresa_reporte = empresas_elegidas_consolidar[0]
            elif len(empresas_elegidas_consolidar) <= 3:
                nombre_empresa_reporte = " + ".join(empresas_elegidas_consolidar)
            else:
                nombre_empresa_reporte = "Consorcio Consolidado"
    ...
    if len(nombre_empresa_limpio) > 30:
        nombre_empresa_limpio = "Consorcio_Comparativo" if modo_analisis == "Comparar Grupos/Empresas" else "Consorcio_Consolidado"
        
    nombre_archivo = f"Reporte_{fluido_archivo}_{tipo_archivo}_{nombre_empresa_limpio}.xlsx"
    
    st.markdown("---")
    st.download_button(
        label="👨‍💻Descargar Archivo Excel Definitivo",
        data=buffer,
