...
if st.button("Procesar Datos y Generar Excel", type="primary"):
    ...
    # --- INSCRIPCIÓN DE FUENTES Y MÉTODOS ---
    ...
    if es_petroleo:
        texto_calculo = "• Cálculo Petróleo: Se multiplican los metros cúbicos (m3) por 6,28981 y se dividen por 1000 para obtener Miles de Barriles (Kbbl)."
    else:
        texto_calculo = "• Cálculo Gas: Se dividen los miles de metros cúbicos (Mm3) por 6 y luego por 1000 para obtener Miles de Barriles Equivalentes de Petróleo (KBoe)."
        
    ws.cell(row=fila_notas + 3, column=1, value=texto_calculo)
    ...
    # El gráfico solo referenciará la data, no las notas del pie
    datos_ref = Reference(ws, min_col=indice_col_final, min_row=1, max_col=indice_col_final, max_row=ws.max_row - 4)
    cat_ref = Reference(ws, min_col=1, min_row=2, max_row=ws.max_row - 4)
    ...
    st.success("¡El reporte financiero ha sido forjado con éxito y con la bibliografía al pie!")
