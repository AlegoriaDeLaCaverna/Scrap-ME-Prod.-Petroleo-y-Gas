...
with col2:
    ...
    empresas_elegidas = st.multiselect(
        "🏭Seleccione Empresas a auditar (Deje vacío para analizar Todas):", 
        options=lista_empresas_reales
    )
    
    # El nuevo interruptor táctico
    modo_analisis = "Consolidar (Sumar total)"
    if len(empresas_elegidas) > 1:
        modo_analisis = st.radio("⚔️ Modo de Análisis Corporativo:", ["Consolidar (Sumar total)", "Comparar Empresas"])
    
    if es_desglosable and modo_analisis == "Consolidar (Sumar total)":
...
if st.session_state.procesar_clicked:
    ...
    if not empresas_elegidas:
        ...
        df_filtrado = df_filtrado[df_filtrado['empresa'].isin(empresas_elegidas)]
        if modo_analisis == "Comparar Empresas":
            nombre_empresa_reporte = " vs ".join(empresas_elegidas[:3])
            if len(empresas_elegidas) > 3:
                nombre_empresa_reporte += " y otras"
        else:
            if len(empresas_elegidas) == 1:
                nombre_empresa_reporte = empresas_elegidas[0]
            elif len(empresas_elegidas) <= 3:
                nombre_empresa_reporte = " + ".join(empresas_elegidas)
            else:
                nombre_empresa_reporte = "Consorcio de Empresas"
    ...
    # --- BIFURCACIÓN DEL MOTOR SEGÚN EL MODO ELEGIDO ---
    if modo_analisis == "Comparar Empresas":
        col_valor = cols_metricas[0]
        
        # Agrupamos manteniendo la identidad de cada empresa
        df_agrupado = df_filtrado.groupby(['Periodo', 'empresa'])[col_valor].sum().reset_index()
        df_agrupado = df_agrupado.pivot(index='Periodo', columns='empresa', values=col_valor).reset_index().fillna(0)
        df_agrupado.columns.name = None
        
        cols_empresas = [c for c in df_agrupado.columns if c != 'Periodo']
        cols_convertidas = []
        
        for emp in cols_empresas:
            col_conv = f"{emp} ({acronimo_final})"
            df_agrupado[col_conv] = (df_agrupado[emp] * factor_conversion).round(2)
            cols_convertidas.append(col_conv)
            
        columnas_finales = ['Periodo'] + cols_empresas + cols_convertidas
        df_agrupado = df_agrupado[columnas_finales]
        cols_to_graph_web = cols_convertidas
        
        encabezados_excel = ['Período'] + [f"{emp} (Orig)" for emp in cols_empresas] + cols_convertidas

    else:
        # Lógica original de consolidación absoluta
        ...
        encabezados_excel = ['Período'] + [str(col).replace('_', ' ').title() for col in cols_grafico]
        if se_puede_desglosar:
            encabezados_excel += [col_conv_convrt, col_noconv_convrt, nombre_col_nueva]
        else:
            encabezados_excel += [nombre_col_nueva]

    # --- VITRINA DE PREVISUALIZACIÓN WEB ---
    st.markdown("---")
    st.subheader(f"📊Vista Previa: {nombre_fluido} - {nombre_empresa_reporte}")
    ...
    if modo_analisis == "Comparar Empresas":
        min_c = len(columnas_finales) - len(cols_convertidas) + 1
        max_c = len(columnas_finales)
        chart.legend.position = 'b'
    elif se_puede_desglosar and tipo_grafico == "Convencional vs No Convencional":
    ...
    nombre_empresa_limpio = nombre_empresa_reporte.replace(' ', '_').replace('/', '-')
    if len(nombre_empresa_limpio) > 30:
        nombre_empresa_limpio = "Consorcio_Comparativo" if modo_analisis == "Comparar Empresas" else "Consorcio_Consolidado"
